...
print(f"✓ Excel loaded: {len(xdf_data)} rows")

## Multiclass - verify the classes, change from strings to numbers
class_names = process_target(1)
print(f"✓ Target processed - {len(class_names)} classes: {class_names}")

# Filtering the information
xdf_dset = xdf_data[xdf_data["split"] == SPLIT].copy()
print(f"✓ Split '{SPLIT}' selected: {len(xdf_dset)} samples\n")

# Processing the information (create dataset)
test_ds = read_data(len(class_names))
print(f"✓ Dataset created\n")

# IMPORTANT: Call predict_func FIRST to add results column
print("="*60)
print("MAKING PREDICTIONS")
print("="*60)
predict_func(test_ds)

# THEN call metrics_func to evaluate
print("\n" + "="*60)
print("CALCULATING EVALUATION METRICS")
print("="*60)
metrics_func(['f1_macro', 'f1_weighted', 'coh', 'acc', 'mat'], ['avg', 'sum'])
print("="*60)